...
# Initialize tokenizer from local directory or Hugging Face model repository
tokenizer = DistilBertTokenizer.from_pretrained('distilbert-base-uncased')

# Load silicone dataset, pick yours here
silicone_dataset = load_dataset('silicone', 'dyda_e')
...
def encode_data_with_context(df, window_size, context_type):
    ...
    for index, row in df.iterrows():
        ...
        if context_type in ['previous', 'both']:
            previous_context = ' '.join(df[(df['Dialogue_ID'] == current_dialogue_id) & (df.index < index)].tail(window_size)['Utterance'])
            context_string += previous_context + " "

        # Add target utterance
        context_string += row['Utterance'] + " "

        # Add subsequent context
        if context_type in ['subsequent', 'both']:
            subsequent_context = ' '.join(df[(df['Dialogue_ID'] == current_dialogue_id) & (df.index > index)].head(window_size)['Utterance'])
